...
- [二、1.0 详细计划（当前需求）](#二10-详细计划当前需求)
  ...
  - [Phase 1.0.6 端到端打通（Week 9，1.0 GA 后补丁）](#phase-106-端到端打通week-91-ga-后补丁)
...
| Phase 1.0.6 完成 | `1.0.0` → `1.0.1`     | patch（补丁）  | `v1.0.1`（正式 release） |
...
| **D. 模型处理** |                              |                                                                                                          |       |                                 |
| T1.0.3.11       | 默认模型省略策略             | outbound provider 请求不主动设置 `model`；依赖供应商默认模型；保留 inbound `model` 仅用于日志 / 兼容解析 | 0.25d | T1.0.3.01                       |
| T1.0.3.12       | 供应商默认模型文案           | UI / 文档明确“模型由供应商端默认配置决定”，不提供本地模型映射表                                          | 0.25d | T1.0.3.11                       |
| **E. 测试**     |                              |                                                                                                          |       |                                 |
...
### Phase 1.0.6 端到端打通（Week 9，1.0 GA 后补丁）

**背景**：1.0.0 GA 出货后发现 `/v1/chat/completions`、`/v1/messages`、`/v1/models` 三个对外 HTTP 端点仍是 Phase 1.0.1 留下的 stub（`apps/desktop/src-tauri/src/proxy/server.rs:222-239`），客户端配置完 BASE URL 与 sk-local Key 后实际打过去全部返回 `503 providers_not_configured`。后端 `SwitchEngine` / `ProviderManager` / 三家 converter / SSE 转发 / 健康检查全部就绪，仅差**接线**与若干闭环（启动加载、热重载、供应商默认模型兜底策略、错误形态适配、E2E 验证）。本 Phase 把这些闭环补齐，让"添加供应商 → 客户端真实请求 → 自动切换 → 监控反馈"端到端可用。

**目标**：`/v1/*` 三端点真实跑通；UI 添加 / 修改 / 删除 provider 不重启即生效；OpenAI / Anthropic 两种 inbound 形态（含工具调用与流式）均可路由到任意 outbound provider；故障注入下切换链路与 UI 反馈一致。

**版本号变化**：`1.0.0` → `1.0.1`（patch；GA 后第一个补丁，**正式 release，非 pre-release**）

**任务清单**（共 34 个，预计 9.75 人日）：

| ID                          | 任务                          | 说明                                                                                                                                                                                          | 工时  | 依赖                 |
| --------------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | -------------------- |
| **A. 后端接线（核心）**     |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.01                   | ProxyRuntime 注入依赖         | `ProxyRuntime::new` / `start` 接受 `Arc<SwitchEngine>` + `Arc<ProviderManager>`；保存到内部 state                                                                                             | 0.25d | T1.0.1.06            |
| T1.0.6.02                   | build_router 持有 AppState    | 把三个 handle 通过 `axum::Extension` 或 `with_state` 注入；handler 签名改为 `State<AppState>`                                                                                                 | 0.25d | T1.0.6.01            |
| T1.0.6.03                   | ProviderError → ApiError 映射 | 透传 status；按 inbound 协议（OpenAI / Anthropic）输出对应 error envelope；保留 `request-id`                                                                                                  | 0.25d | T1.0.2.16            |
| T1.0.6.04                   | 启动时加载 provider           | `lib.rs` setup 中 `manager.load_from_repository(repo).await`；失败仅警告不阻塞                                                                                                                | 0.1d  | T1.0.1.16            |
| **B. /v1/chat/completions** |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.05                   | 非流式 chat_completions       | 解析 → `OpenAIConverter::request_to_unified` → `engine.dispatch` → 选 outbound converter `unified_to_response` → `Json(Value)`                                                                | 0.5d  | T1.0.6.02, T1.0.3.\* |
| T1.0.6.06                   | 流式 chat_completions         | `stream=true` 走 `engine.dispatch_stream` + `sse::stream_to_sse_response`；逐 chunk `parse_stream_chunk → encode_stream_chunk`                                                                | 0.5d  | T1.0.6.05            |
| T1.0.6.07                   | chat_completions 集成测试     | wiremock 模拟 OpenAI / Anthropic outbound；覆盖普通文本、tools、流式、429 重试切换                                                                                                            | 0.5d  | T1.0.6.06            |
| **C. /v1/messages**         |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.08                   | 非流式 anthropic_messages     | `AnthropicConverter::request_to_unified` → dispatch → outbound converter；保留 `system` 字段语义                                                                                              | 0.5d  | T1.0.6.02, T1.0.3.04 |
| T1.0.6.09                   | 流式 anthropic_messages       | 输出 Anthropic SSE 事件序列（`message_start` / `content_block_delta` / `message_stop`），与官方 SDK 对齐                                                                                      | 0.5d  | T1.0.6.08            |
| T1.0.6.10                   | anthropic_messages 集成测试   | wiremock；覆盖 tool_use 双向、`stop_reason`、流式 chunk 顺序                                                                                                                                  | 0.5d  | T1.0.6.09            |
| **D. /v1/models 聚合**      |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.11                   | /v1/models 聚合实现           | 遍历 `enabled_by_priority()` 并发查每家 `/v1/models`；按 `provider_id::model_id` 命名；30s in-memory 缓存                                                                                     | 0.5d  | T1.0.6.02            |
| T1.0.6.12                   | /v1/models 测试               | 多 provider 去重、单 provider 失败时降级返回部分结果、空清单兜底                                                                                                                              | 0.25d | T1.0.6.11            |
| **E. Provider Hot-reload**  |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.13                   | manager.reload 公共方法       | 在 `ProviderManager` 上加 `reload_from_repository(&self, repo)`；写时锁、保留运行中健康状态                                                                                                   | 0.25d | T1.0.2.03            |
| T1.0.6.14                   | CRUD 命令触发热重载           | `add_provider` / `update_provider` / `delete_provider` 写库后调用 reload；失败回滚 DB 提交                                                                                                    | 0.25d | T1.0.6.13            |
| T1.0.6.15                   | 热重载测试                    | Tauri command 单元测试 + UI 集成：添加供应商后 `enabled_by_priority` 立即包含新条目                                                                                                           | 0.25d | T1.0.6.14            |
| **F. 供应商默认模型接通**   |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.16                   | outbound 默认模型兜底         | handler / converter 允许入站省略 `model`；Provider 优先保留客户端模型，缺省或模型选择错误时再按供应商类型补默认模型，避免上游 `model is required` 或代理改写模型导致账号池错误                | 0.25d | T1.0.3.12, T1.0.6.05 |
| T1.0.6.17                   | 默认模型策略测试              | 覆盖：chat_completions / anthropic_messages outbound body 均不含 `model`；供应商返回正常时响应仍可按 inbound 协议编码                                                                         | 0.25d | T1.0.6.16            |
| **G. 错误响应与中间件**     |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.18                   | 协议感知错误响应              | `IntoResponse for ApiError` 根据请求 path 选 OpenAI / Anthropic 错误 envelope                                                                                                                 | 0.25d | T1.0.6.03            |
| T1.0.6.19                   | 请求体大小 / 超时中间件       | 1 MB body 上限（OpenAI 兼容）；handler 默认 600s 超时（流式不计入）；超出走 4xx                                                                                                               | 0.25d | T1.0.6.02            |
| **H. E2E + 文档**           |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.20                   | Playwright + 真请求 E2E       | UI 添加 mock provider → 启动代理 → curl `/v1/chat/completions` → 监控面板出现请求条目                                                                                                         | 0.5d  | T1.0.5.10, T1.0.6.15 |
| T1.0.6.21                   | 故障注入复检                  | 复用 T1.0.5.09 框架，验证 503 stub 移除后 degraded 标记、UI 切换提示、重试链行为                                                                                                              | 0.25d | T1.0.5.09, T1.0.6.07 |
| T1.0.6.22                   | 用户手册补"已支持端点"矩阵    | OpenAI / Anthropic compat 矩阵（端点 × 字段 × 流式 × 工具调用）追加到中英文档                                                                                                                 | 0.25d | T1.0.5.14, T1.0.6.10 |
| T1.0.6.23                   | README 加快速验证 curl 样例   | `curl -H "Authorization: Bearer sk-local-..." http://127.0.0.1:8787/v1/chat/completions` 完整可复制片段                                                                                       | 0.1d  | T1.0.6.22            |
| T1.0.6.24                   | 任务报告补登修订              | 在 `docs/任务报告.md` 末尾用 `## 修订` 段说明 T1.0.2.15 / T1.0.3.04 接线在 1.0.6 才完成（不修改历史记录）                                                                                     | 0.1d  | —                    |
| **I. 版本与发布**           |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.25                   | 版本号跳变                    | `tauri.conf.json.version` `1.0.0` → `1.0.1`；`Cargo.toml` 同步                                                                                                                                | 0.1d  | —                    |
| T1.0.6.26                   | CHANGELOG `1.0.1` 节          | 主条目：`fix(proxy): wire /v1/* HTTP routes to SwitchEngine`；附用户可见的端点新增清单                                                                                                        | 0.1d  | T1.0.6.25            |
| T1.0.6.27                   | 触发 1.0.1 正式 release       | 合 main → tag `v1.0.1` → 三平台产物 + 公证 / 签名；**不再带 prerelease 标记**                                                                                                                 | 0.1d  | T1.0.6.25, T1.0.5.20 |
| **J. Review 遗留问题修复**  |                               |                                                                                                                                                                                               |       |                      |
| T1.0.6.28                   | 健康状态事件推送前端          | `HealthChecker` 状态变更时调用 `app_handle.emit("provider-status-changed", payload)`，前端通过 `@tauri-apps/api/event.listen()` 接收并刷新 UI（修复 T1.0.2.08）                               | 0.25d | T1.0.2.08, T1.0.6.04 |
| T1.0.6.29                   | 负载均衡改为加权 round-robin  | `LoadBalanceStrategy` 按 `priority` 倒数作为权重分配请求比例，替换当前的简单轮询（修复 T1.0.2.13）                                                                                            | 0.25d | T1.0.2.13            |
| T1.0.6.30                   | Smart 权重滑块联动            | 4 个 `WeightSlider` 总和强制 = 100；调整其中一个时按比例缩放其余三个；显示实时总和校验（修复 T1.0.4.08）                                                                                      | 0.25d | T1.0.4.08            |
| T1.0.6.31                   | 托盘图标替换 + Windows .ico   | 替换 68 字节占位符为真实设计图标；补充 `icon.ico`（256×256 含多尺寸层）用于 Windows 任务栏和系统托盘（修复 T1.0.4.16）                                                                        | 0.25d | T1.0.4.16            |
| T1.0.6.32                   | Playwright Tauri E2E 测试     | 安装 `@playwright/test` + `tauri-driver`；覆盖：启动 → 添加供应商 → 发请求 → 监控面板验证；CI 配置 headless 运行（补齐 T1.0.5.10）                                                            | 1d    | T1.0.6.20            |
| T1.0.6.33                   | 干净环境冒烟测试              | macOS + Windows 各一台干净虚拟机安装 1.0.1 安装包；验证首次启动、添加供应商、请求转发、托盘、关闭到托盘全链路；记录结果到 `docs/smoke-test-report.md`（补齐 T1.0.5.21）                       | 0.5d  | T1.0.6.27            |
| T1.0.6.34                   | 协议分离本地凭据与供应商分组  | 本地代理分别签发 OpenAI-compatible / Anthropic Key；Dashboard 分开展示 Base URL/Key；Provider 列表按 OpenAI-compatible / Anthropic 分组；Relay/Custom 作为 OpenAI-compatible 可添加           | 0.25d | T1.0.6.18, T1.0.2.22 |
| T1.0.6.35                   | 未匹配 /v1/\* 返回 404        | `ApiErrorKind::NotFound` + `ApiError::not_found()`；`build_router` 给 v1 子 router 加 `fallback` → 协议感知 404 envelope；测试覆盖带 / 不带 key 与 `/v1/messages/*` 子路径                    | 0.25d | T1.0.6.18, T1.0.6.34 |
| T1.0.6.36                   | ResponsesConverter 入站解析   | 新建 `converter/responses.rs`；`request_to_unified` 覆盖 `input` 字符串 / 字符串数组 / 类型化 item，`instructions` prepend，自定义 function tools 透传，内置工具拒绝；≥12 条单元测试          | 0.75d | T1.0.6.35            |
| T1.0.6.37                   | ResponsesConverter 响应序列化 | `unified_to_response` 输出 `{id, object:"response", created_at, status, model, output[], usage, ...}`；text → message 项 + function_call 项；Length/ContentFilter → incomplete；≥6 条单元测试 | 0.5d  | T1.0.6.36            |
| T1.0.6.38                   | ResponsesStreamEncoder        | 流式状态机驱动 9 类典型 SSE 事件 + `error`；text 与 tool_call item 生命周期管理；≥8 条单元测试覆盖完整序列 / 切换 / 异常 / Length / 空流 / usage / 帧格式                                     | 1d    | T1.0.6.37            |
| T1.0.6.39                   | /v1/responses 路由 wiring     | `ProxyAppState.responses_converter`；`responses_create` handler（非流式 + 流式）；流式 helper `openai_stream_to_responses_sse`；`ResponsesStreamState` 状态结构；≥7 条 wiremock 集成测试      | 0.5d  | T1.0.6.38, T1.0.6.18 |
| T1.0.6.40                   | Responses 文档同步            | `docs/user-manual.md`（中英）端点矩阵；`README.md` / `README.zh-CN.md` curl 样例；`docs/产品技术文档.md` 端点与协议说明；本表追加 T1.0.6.35–41                                                | 0.25d | T1.0.6.39            |
| T1.0.6.41                   | Dashboard 总览按协议切换      | 顶部 4 张状态卡上方加 antd Segmented 切换 OpenAI / Anthropic；`HealthSnapshot` 加 `kind`；`get_metrics_timeseries` 加 `protocol` 参数按 kind 列表 JOIN providers 过滤；新增 1 条 Tab 切换测试 | 0.5d  | T1.0.6.34            |

**交付物**：

- GitHub Release `v1.0.1`（正式 release，三平台签名包）
- 任务报告 `## 修订` 段：T1.0.2.15 / T1.0.3.04 实际接线时间
- CHANGELOG `1.0.1` 节、用户手册"已支持端点"矩阵
- 新增集成测试：`tests/proxy_e2e.rs`（HTTP→SwitchEngine→Provider 全链路）
- Playwright E2E 测试套件（UI 全链路覆盖）
- 干净环境冒烟测试报告（`docs/smoke-test-report.md`）
- 真实设计的托盘图标（含 Windows .ico）
- `/v1/responses` OpenAI Responses API 端点（非流式 + 类型化 SSE）

**验收标准**：

- [ ] 干净系统装 1.0.1 → UI 添加任一 OpenAI / Anthropic 兼容供应商 → 客户端用 `BASE URL` + `sk-local-…` 直接打 `/v1/chat/completions` 与 `/v1/messages` 都返回 200 + 真实响应
- [ ] `stream=true` 时浏览器 / SDK 能正确解析 SSE 事件（OpenAI `data: {…}\n\n` + Anthropic `event: …\ndata: {…}\n\n` 两种形态）
- [ ] `/v1/models` 聚合返回所有已启用 provider 的模型清单
- [ ] UI 添加 / 删除 provider 后**无需重启**，下一个请求立即生效
- [ ] 故障注入（429 / 5xx）触发自动切换并在监控面板可见 degraded 标记
- [ ] `cargo test` 全绿；`tests/proxy_e2e.rs` 至少 6 条用例通过
- [ ] 503 `providers_not_configured` 仅在"完全没有 enabled provider"时返回，**不再因接线缺失出现**
- [ ] 健康状态变更时前端 UI 实时刷新（`provider-status-changed` 事件推送正常）
- [ ] 负载均衡策略按优先级倒数加权分配，非简单轮询
- [ ] Smart 策略四维权重滑块总和恒等于 100，调整时联动
- [ ] 托盘图标在 macOS / Windows 均清晰显示（非占位符）
- [ ] Playwright E2E 在 CI 中 headless 运行通过
- [ ] macOS + Windows 干净虚拟机冒烟测试通过并有记录
- [ ] 客户端用 OpenAI Key 直接打 `/v1/responses` 能拿到 `{object:"response", status:"completed", output:[...]}`；`stream=true` 时输出完整 Responses 类型化事件序列（`response.created` → `response.in_progress` → `response.output_item.added` → `response.content_part.added` → `response.output_text.delta`\* → `response.output_text.done` → `response.content_part.done` → `response.output_item.done` → `response.completed`，上游异常发 `error`）
- [ ] 未注册 `/v1/*` 路径在客户端带正确 key 时返回 404 `not_found`（不再误报 401 invalid api key）
- [ ] Dashboard 顶部「总览」4 张状态卡可按 OpenAI / Anthropic 协议切换，4 项指标全部按所选协议重算
...
| T1.0.W.TURBO    | Turborepo 接入         | 引入 turbo + Vercel Remote Cache；root scripts 全部走 turbo filter      | 0.3d  | TW.1.05   |
...
| 供应商路由             | 按用户配置的规则匹配供应商；请求模型名可省略，省略时由 Provider 使用默认模型兜底 |
...
Phase 1.0.6 端到端打通（1.0 GA 后补丁）
  - [ ] ProxyRuntime / build_router 注入 SwitchEngine + ProviderManager
  - [ ] 启动加载 + Provider CRUD 热重载
  - [ ] /v1/chat/completions 非流式 + 流式真实接通
  - [ ] /v1/messages 非流式 + 流式真实接通（Anthropic SSE 事件序列）
  - [ ] /v1/models 聚合 + 30s 缓存
  - [ ] outbound provider 请求默认模型兜底
  - [ ] 协议感知错误响应 + 请求体大小 / 超时中间件
  - [ ] HTTP→SwitchEngine→Provider E2E + 故障注入复检
  - [ ] tauri.conf.json version → 1.0.1，正式 release
