...
**背景**：1.0.0 GA 出货后发现 `/v1/chat/completions`、`/v1/messages`、`/v1/models` 三个对外 HTTP 端点仍是 Phase 1.0.1 留下的 stub（`apps/desktop/src-tauri/src/proxy/server.rs:222-239`），客户端配置完 BASE URL 与 sk-local Key 后实际打过去全部返回 `503 providers_not_configured`。后端 `SwitchEngine` / `ProviderManager` / 三家 converter / SSE 转发 / 健康检查全部就绪，仅差**接线**与若干闭环（启动加载、热重载、模型映射注入、错误形态适配、E2E 验证）。本 Phase 把这些闭环补齐，让"添加供应商 → 客户端真实请求 → 自动切换 → 监控反馈"端到端可用。
...
**任务清单**（共 33 个，预计 9.5 人日）：

| ID                          | 任务                          | 说明                                                                                                                                                                    | 工时  | 依赖                 |
| --------------------------- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | -------------------- |
| **A. 后端接线（核心）**     |                               |                                                                                                                                                                         |       |                      |
| T1.0.6.01                   | ProxyRuntime 注入依赖         | `ProxyRuntime::new` / `start` 接受 `Arc<SwitchEngine>` + `Arc<RwLock<ModelMapping>>` + `Arc<ProviderManager>`；保存到内部 state                                         | 0.25d | T1.0.1.06            |
...
| **F. Model Mapping 接通**   |                               |                                                                                                                                                                         |       |                      |
| T1.0.6.16                   | 转 unified 后应用映射         | handler 在 `request_to_unified` 后调用 `model_mapping.resolve(&inbound, &provider_id)`；未命中走原模型名                                                                | 0.25d | T1.0.3.12, T1.0.6.05 |
| T1.0.6.17                   | 模型映射测试                  | 覆盖：精确命中、provider 通配、无映射兜底；与 chat_completions / anthropic_messages 各跑 1 次                                                                           | 0.25d | T1.0.6.16            |
| **G. 错误响应与中间件**     |                               |                                                                                                                                                                         |       |                      |
| T1.0.6.18                   | 协议感知错误响应              | `IntoResponse for ApiError` 根据请求 path 选 OpenAI / Anthropic 错误 envelope                                                                                           | 0.25d | T1.0.6.03            |
| T1.0.6.19                   | 请求体大小 / 超时中间件       | 1 MB body 上限（OpenAI 兼容）；handler 默认 60s 超时（流式不计入）；超出走 4xx                                                                                          | 0.25d | T1.0.6.02            |
| **H. E2E + 文档**           |                               |                                                                                                                                                                         |       |                      |
...
**目标**：先完成 Vercel 可部署配置；生产域名、DNS、正式部署操作与发布日验收后置，由仓库 owner 在 Vercel 执行。

**任务清单**（共 8 个，预计 3 人日）：

| ID      | 任务              | 说明                                                    | 工时  | 依赖      |
| ------- | ----------------- | ------------------------------------------------------- | ----- | --------- |
| TW.3.01 | Vercel 配置       | Vercel root / build / install / output / headers        | 0.25d | —         |
| TW.3.02 | Analytics         | Plausible / Vercel Analytics（不收集 PII）              | 0.25d | TW.3.01   |
| TW.3.03 | 站点性能准备      | `next build` 通过；图片用 `next/image`；Lighthouse 后置 | 0.5d  | TW.1.\*   |
| TW.3.04 | 跨浏览器测试清单  | Chrome / Safari / Firefox / Edge 验收清单，人工测试后置 | 0.25d | TW.3.01   |
...
| TW.3.08 | GA 上线准备       | Vercel 部署说明；DNS / 社媒 / 首日观测后置              | 0.5d  | TW.3.07   |

**交付物**：

- `apps/website/vercel.json` 可被 Vercel 项目读取
- `docs/website-launch-checklist.md` 记录 Vercel 项目配置和后置事项
- 与桌面端 `v1.0.0` 的官网 / 下载页入口已在代码侧接好

**验收标准**：

- [ ] `next build` 通过；Lighthouse 全维度 ≥ 90 后置到真实 Vercel URL
- [ ] 移动端可用，无横向滚动
- [ ] 隐私 / 服务条款齐备
...
不新增 GitHub Actions 部署 workflow；官网由 Vercel Git 集成或 Vercel 控制台部署。

- **Root Directory**：`apps/website`
- **Install Command**：`cd ../.. && pnpm install --frozen-lockfile`
- **Build Command**：`cd ../.. && pnpm --filter @ccuse/website build`
- **Output Directory**：`.next`
- **环境变量**：`NEXT_PUBLIC_SITE_URL` 可留空；未绑定自定义域名时会优先使用 Vercel 注入的 URL
...
Phase 1.0.6 端到端打通（1.0 GA 后补丁）
  - [ ] ProxyRuntime / build_router 注入 SwitchEngine + ModelMapping + ProviderManager
  - [ ] 启动加载 + Provider CRUD 热重载
  - [ ] /v1/chat/completions 非流式 + 流式真实接通
  - [ ] /v1/messages 非流式 + 流式真实接通（Anthropic SSE 事件序列）
  - [ ] /v1/models 聚合 + 30s 缓存
  - [ ] ModelMapping 注入到 handler
