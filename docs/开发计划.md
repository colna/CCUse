...
**背景**：1.0.0 GA 出货后发现 `/v1/chat/completions`、`/v1/messages`、`/v1/models` 三个对外 HTTP 端点仍是 Phase 1.0.1 留下的 stub（`apps/desktop/src-tauri/src/proxy/server.rs:222-239`），客户端配置完 BASE URL 与 sk-local Key 后实际打过去全部返回 `503 providers_not_configured`。后端 `SwitchEngine` / `ProviderManager` / 三家 converter / SSE 转发 / 健康检查全部就绪，仅差**接线**与若干闭环（启动加载、热重载、供应商默认模型省略策略、错误形态适配、E2E 验证）。本 Phase 把这些闭环补齐，让"添加供应商 → 客户端真实请求 → 自动切换 → 监控反馈"端到端可用。
...
| **F. 供应商默认模型接通**   |                               |                                                                                                                                                                         |       |                      |
| T1.0.6.16                   | outbound 省略 model           | handler / converter 转 provider 请求时移除 `model` 字段；请求日志可记录 inbound `model`，但不向供应商设置模型                                                           | 0.25d | T1.0.3.12, T1.0.6.05 |
...
| 供应商路由             | 按用户配置的规则匹配供应商；不根据请求模型名给供应商设置模型  |
