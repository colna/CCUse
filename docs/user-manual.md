...
| `POST /v1/responses`        | OpenAI Responses     | `model`, `input` (string / array of strings / typed items), `instructions`, `tools` (function only), `temperature`, `top_p`, `max_output_tokens`, `stop`, `stream` | `id` (`resp_…`), `object: "response"`, `created_at`, `status`, `model`, `output[]`, `usage` | Yes. Emits typed events: `response.created`, `response.in_progress`, `response.output_item.added`, `response.content_part.added`, `response.output_text.delta`, `response.output_text.done`, `response.content_part.done`, `response.output_item.done`, `response.completed`, and `error` on upstream failures. | Yes (custom `type: "function"` tools only). Builtin tools (`web_search`, `file_search`, `code_interpreter`, `image_generation`, `mcp`, `computer_use`) are rejected with HTTP 400. | `previous_response_id`, `store`, `text.format`, `reasoning`, `metadata` are silently ignored (stateless proxy). The response `model` field echoes the client's request so account-pool model rewrites don't break correlation. |

Currently unsupported local API surfaces: embeddings, image generation, audio, files, batches, and fine-tuning endpoints.
...
| `POST /v1/responses`        | OpenAI Responses   | `model`、`input`（字符串 / 字符串数组 / 类型化 item 数组）、`instructions`、`tools`（仅 `type:"function"`）、`temperature`、`top_p`、`max_output_tokens`、`stop`、`stream` | `id`（`resp_…`）、`object: "response"`、`created_at`、`status`、`model`、`output[]`、`usage` | 支持；输出类型化事件序列：`response.created`、`response.in_progress`、`response.output_item.added`、`response.content_part.added`、`response.output_text.delta`、`response.output_text.done`、`response.content_part.done`、`response.output_item.done`、`response.completed`；上游异常时发送 `error` | 支持自定义 `type:"function"` 工具；内置工具（`web_search`、`file_search`、`code_interpreter`、`image_generation`、`mcp`、`computer_use`）返回 HTTP 400。 | `previous_response_id`、`store`、`text.format`、`reasoning`、`metadata` 静默忽略（无状态代理）；响应 `model` 字段回显客户端请求，避免账号池服务器改写 model 名时影响客户端对齐请求/响应。 |

当前未支持的本地 API 面：embeddings、图像生成、音频、文件、batch 与 fine-tuning 端点。
