...
模型策略：代理不向 outbound provider 主动设置 `model` 字段；供应商实际使用的模型由供应商端默认配置决定。`inbound_model` 仅用于兼容解析、日志和客户端请求回放，不参与供应商请求体构造。
...
模型字段处理：入站请求中的 `model` 可以被解析并记录，但转发给供应商时默认省略，避免代理覆盖供应商侧默认模型。
