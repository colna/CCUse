...
模型策略：客户端可以省略 `model`；即使客户端传入 `model`，代理也不把它用于 outbound provider 请求，而是在转发前按供应商协议补齐默认模型，避免上游返回 `400 model is required`。OpenAI-compatible 默认按 `gpt-5.5` → `gpt-5.5-instant` → `gpt-5.5-instant` 尝试，Anthropic 默认按 `claude-opus-4.7` → `claude-sonnet-4.6` → `claude-haiku-4-5` 尝试；Relay / Custom 会使用包含 GPT 与 Claude 的 OpenAI-compatible 默认候选链。`inbound_model` 仅用于兼容解析、日志和客户端请求回放。
