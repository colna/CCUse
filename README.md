...
供应商配置会写入本机应用数据库，API Key 加密后保存；正常重启应用不会清空已添加供应商。macOS 端当前使用应用本地 master key 文件，避免反复触发系统钥匙串访问弹窗。

健康检查采用偏实战的 OpenAI-compatible 探测方式：优先请求 `/v1/models`；如果中转商不支持模型列表或返回非认证类失败，再用极小的 `/v1/chat/completions` 请求探测真实补全链路。401 / 403 会标记为 Down，429 / 5xx 会标记为 Degraded，探测模型被 400 / 404 / 422 拒绝时保留为 Degraded，避免把“模型不匹配”误判成整家供应商不可用。

自动切换会优先选择 Healthy，其次 Degraded；如果剩余供应商都被健康检查标记为 Down，仍会按策略最后尝试一次，因为部分中转商的健康端点可能不完整。真实请求中的 401 / 403 / 429 / 5xx / 网络错误会进入同一次请求的重试链，并写入 `request_logs` 与 `switch_history`，方便在监控页回看是哪家供应商失败、最终切到了哪家。

仪表盘里的“当前供应商”指最近一次真实代理请求实际路由到的供应商，而不是供应商列表里的第一个健康快照。刚启动且还没有请求记录时显示“无”是正常状态。
