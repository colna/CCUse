...
> 本文档对所有未来在本仓库内的 Codex 会话生效。每次新会话开启后，必须先读取本文件并严格遵循。
> 与全局 `~/.codex/AGENTS.md` 同时生效；本项目规则**优先级更高**。
...
3. **检查 context 用量** —— 见 §4
4. **必要时压缩 context** —— 见 §4
5. **提交代码** —— 见 §7（在 Codex 权限与沙箱策略允许范围内执行；如界面要求审批，按 Codex 审批流程处理）
...
- **Context 检查**: x%（>40% 已执行 compact；如当前 Codex 界面不提供显式 context 百分比，记录可见的 token/上下文状态）
- **风险 / 后续 todo**: 无 / xxx
- **备注**: xxx
```
...
**每完成 1 个 task** 后立即检查（即使 task 很小也要检查）。检查方式：优先使用 Codex 界面提供的 context/token 指示；如当前界面支持 `/context`，可以调用 `/context`。
...
| ------------ | --------------------------------------------------------------------------------- |
| ≤ 40%        | 继续下一个 task                                                                   |
| > 40%        | 立即执行 compact（如当前界面支持 `/compact`，使用 `/compact`）后再开始下一个 task |
| ≥ 80%        | 强制 compact；如压缩后仍 ≥ 60%，停下并告知用户                                    |

### 4.3 压缩前必须做

- 把当前 task 的报告写入 `docs/任务报告.md`（避免压缩丢上下文）
- 把进行中的 task ID、文件路径、待办点写入 `.codex/scratchpad.md`（如不存在则创建）
- 然后再 compact
...
凡涉及 **UI / UX / 视觉 / 交互**（无论桌面端、官网，或文档示意图），必须按顺序使用以下 skill 后才动手：
...
>
> 如果 Codex 当前会话没有加载对应 skill，应先检查 `.agents/skills/` 是否存在对应目录；不得用记忆 / 猜测内容代替 skill 输出。
...
提交任何代码前，必须根据语言/框架先使用对应 best-practices skill 获取最新规范，然后按规范写代码 / review 代码：
...
继承全局 `~/.codex/AGENTS.md` 的所有规则，并在此基础上叠加：

| 维度       | 规则                                                                                                                                                                                                 |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 语言       | 中文回复；技术术语 / 标识符保持原文                                                                                                                                                                  |
| 注释       | 默认不写；只在 _为什么_ 不显然时加一行                                                                                                                                                               |
| 提交       | Conventional Commits；**每完成一个 task 立即 `git commit + git push`**。一个 task 一个或多个原子 commit，commit message 含对应 task ID（如 `T1.0.1.07`）；如 Codex 审批/沙箱要求确认，按界面流程处理 |
...
- [ ] 已读取本文件（`CCUse/AGENTS.md`）
- [ ] 已读取 `docs/产品技术文档.md` 与 `docs/开发计划.md` 的相关章节（按当前 task 范围）
- [ ] 已读取 `docs/任务报告.md` 的最近若干条，了解上下文
- [ ] 知道当前要做的 task ID（若用户未指明，先查看 `docs/开发计划.md` 或询问用户）
- [ ] 涉及 UI → 先使用 `apple-design` + `frontend-design`
- [ ] 涉及代码 → 先使用对应语言的 best-practices skill
